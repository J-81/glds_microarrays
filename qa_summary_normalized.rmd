...
## Normalized Box Plot
```{r eval=(plots == "OLIGO"), echo=TRUE, warning=FALSE}
print(data)
#print(targets$t1$SampleName)
#TMP oligo::boxplot(data,which="pm",transfo=log2, nsample=10000,main=paste0(opt$glds," Normalized Intensities"), las=2,outline=FALSE,ylab = "log2 Intensities",names=targets$t1$SampleName)
...
## Normalized PCA Plot
```{r echo=TRUE, message=TRUE, warning=FALSE, error=TRUE,  eval=(targets$labels == 1)}
library(ggplot2)
try(exp_raw <- log2(as.matrix(data)))
try(exp_raw <- log2(exprs(data)))
library("gmodels")
PCA_raw <- fast.prcomp(t(exp_raw), scale. = FALSE)
